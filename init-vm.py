...
def initVM():
    ...
    initProcess = 0
    if os.name == 'nt':
        initProcess = subprocess.Popen(["python3", "-m", "venv", "venv"], shell=True)
    else:
    ...
    if os.name == 'nt':
        ...
        flaskProcess = subprocess.Popen(["pip.exe", "install", "Flask"], shell=True)
        flaskCorProcess = subprocess.Popen(["pip.exe", "install", "flask-cors"], shell=True)
    else:
        os.chdir("venv/bin/")
