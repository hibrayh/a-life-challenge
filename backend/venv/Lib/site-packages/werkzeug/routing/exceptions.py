...
class BuildError(RoutingException, LookupError):
    ...
    def closest_rule(
            self,
            adapter: t.Optional["MapAdapter"]) -> t.Optional["Rule"]:
        def _score_rule(rule: "Rule") -> float:
            return sum([0.98 * difflib.SequenceMatcher(None,
                                                       rule.endpoint,
                                                       self.endpoint).ratio(),
...
class NoMatch(Exception):
    ...
    def __init__(
            self,
            have_match_for: t.Set[str],
            websocket_mismatch: bool) -> None:
