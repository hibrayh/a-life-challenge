...
_unquote_maps: t.Dict[t.FrozenSet[int],
                      t.Dict[bytes, int]] = {frozenset(): _hextobyte}
...
def url_fix(s: str, charset: str = "utf-8") -> str:
    ...
    if s.startswith(
            "file://") and s[7:8].isalpha() and s[8:10] in (":/", "|/"):
        s = f"file:///{s[7:]}"
...
def url_join(
    ...
    scheme, netloc, path, query, fragment = url_parse(
    ...
    while True:
        ...
        n = len(segments) - 1
        while i < n:
            if segments[i] == s("..") and segments[i -
                                                   1] not in (s(""), s("..")):
