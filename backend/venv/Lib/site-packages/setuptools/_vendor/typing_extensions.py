...
# 3.7
    ...
    Final = _FinalForm(
        'Final', doc="""A special typing construct to indicate that a name
...
# 3.7:
    ...
    Literal = _LiteralForm(
        'Literal', doc="""A type that can be used to indicate to type checkers
...
# 3.7
    ...
    class Protocol(metaclass=_ProtocolMeta):
        ...
        # modules. The duplicated code will be removed when Protocol is moved
        # to typing.
                ...
                if gvars is None:
                    ...
                    if not tvarset <= gvarset:
                        ...
                        cls_name = "Generic" if any(
                            b.__origin__ is typing.Generic for b in bases) else "Protocol"
                        raise TypeError(f"Some type variables ({s_vars}) are"
                                        f" not listed in {cls_name}[{s_args}]")
                    tvars = gvars

            initial_bases = bases
            if (extra is not None and isinstance(extra, abc.ABCMeta) and
                    extra not in bases):
                bases = (extra,) + bases
            bases = tuple(_gorg(b) if isinstance(b, GenericMeta) else b
                          for b in bases)
            if any(isinstance(b, GenericMeta)
                   and b is not typing.Generic for b in bases):
    ...
    class AnnotatedMeta(typing.GenericMeta):
        ...
        def __getitem__(self, params):
            ...
            return self.__class__(
                ...
                _no_slots_copy(self.__dict__),
                tvars=_type_vars((tp,)),
                # Metadata is a tuple so it won't be touched by _replace_args
                # et al.
                args=(tp, metadata),
                origin=self,
            )
