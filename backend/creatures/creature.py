...
class Creature:
    def __init__(self, inputGenome, species, id, xCoordinate, yCoordinate, speciesManager, environment):
    ...
    def reproduceSexual(self, otherParent):
        childGenome = genome.createNewGenomeSexual(self.genome, otherParent.genome)
        self.speciesManager.createNewCreature(childGenome)
    ...
    def searchForMate(self, perceivableMates):
        if perceivableMates == []:
            ...
            closestMate = perceivableMates[distances.index(closestMateDistance)]
            degreeOfMovement = math.acos((closestMate.xCoordinate - self.xCoordinate) / closestMateDistance)

            if closestMate.yCoordinate - self.xCoordinate:
                degreeOfMovement = -1 * degreeOfMovement
            
            movementLength = min(self.genome.mobility * MAX_MOVEMENT, closestMateDistance - 1)

            self.moveCreature(degreeOfMovement, movementLength)
        
    
    def performAction(self):
        perceivableEnvironment = self.environment.returnCreaturesPerceivableEnvironment(self)
        actionToPerform = self._decisionNetwork.determineMostFavorableCreatureAction(self, perceivableEnvironment)
                    ...
                    if creature.species == self.species:
                        distanceFromCreature = (math.sqrt((abs(creature.xCoordinate -
                                                    ...
                                                    (abs(creature.yCoordinate -
                                                        self.yCoordinate) ** 2)))
                        distances.append(distanceFromCreature)
                self.reproduceSexual(perceivableEnvironment.perceivableCreatures[distances.index(min(distances))])
