...
function ListSpeciesGenomeInformation(props){
    ...
    const dummySpeciesGenomeInfo = {population: 1, canSee: 'yes', canHear: 'yes'}

    //Can use props.speciesName to also get list of actual Creatures from species
    const dummyCreaturesOfSpeciesList = ["creatureName1", "creatureName2", "creatureName3",]
    ...
    if(showCreatureData){
        ...
        return(
            <>  
                <button className="backButton buttonHover2" onClick={() => {setShowCreatureData(false)}}><FaArrowLeft size={25}/></button>
        ...
        return(
            ...
            <>
                <button className="backButton buttonHover2" onClick={props.toggleView}><FaArrowLeft size={25}/></button>
                {/*This will print out all of the Species genome information */}
                <div>
                    <h1 className="mainTitleFont fillerTitle">{props.speciesName}'s Genome:</h1>
                    <ul>
                        {Object.keys(dummySpeciesGenomeInfo).map((attribute, index) => (
                            <li className="genomeStat">{attribute}: {Object.values(dummySpeciesGenomeInfo)[index]}</li>
                        ))}
                    </ul>
                </div>
        ...
        <>
            <ul>
                {Object.keys(dummyCreatureStats).map((attribute, index) => (
                    <li className="genomeStat">{attribute}: {Object.values(dummyCreatureStats)[index]}</li>
                ))}
            </ul>
        </>
    )
